...
<form class="variations_form cart product_detail" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
	...
	<?php if ( ! empty( $available_variations ) ) : ?>	
		<div class="variations">
				...
				?>
					<!-- <tr><td colspan="2" class="notied"><?php esc_html_e('Please select the required options to display prices', 'wdjewelry');?></td></tr> -->
				<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
					<div>
						...
						<div class="value">
							...
							<option value=""><?php echo esc_html__( 'Escolha uma opção', 'wdjewelry' ) ?>&hellip;</option>
							<?php
								if ( is_array( $options ) ) {
		...
		<div class="button_single_product_wpdance">
			<div class="single_variation_wrap add_to_card_button" style="display:none;">
				<button type="submit" class="single_add_to_cart_button button alt big"><?php echo apply_filters('single_add_to_cart_text', esc_html__( 'Add to cart', 'wdjewelry' ), $product->product_type); ?></button>
			</div>
